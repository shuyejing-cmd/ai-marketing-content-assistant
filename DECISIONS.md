...
## 前端采用 Soft Depth 统一设计系统

- 品牌主色采用组件库蓝色 `#0476D0`，不沿用旧页面的零散黑色主操作。
- 基础组件集中在 `components/ui`，页面通过组合业务组件实现，不继续复制按钮和状态 class。
- 字间距固定为 `0`，不照搬参考组件库中的负字间距。
- 最小点击区域为 44×44px，统一焦点环、加载、禁用、按下反馈和 reduced-motion。
- 卡片圆角以 8px 为上限；Bottom Sheet、圆形图标按钮和胶囊选择器按交互语义例外处理。

## 响应式工作台优先

- 自由生图在宽屏使用会话、结果、配置三栏，不使用营销式 Hero。
- 中等宽度折叠会话栏，手机端使用单列和 Bottom Sheet。
- 上传图片使用独立 Bottom Sheet，不把用户直接带入过长的完整配置面板。
- 模板生图手机端保持单列，商品图使用 `object-contain`，不裁切主体。
- 手机上传预览最大高度 220px，以确保短屏中的状态、预览、移除和完成操作不互相遮挡。

## 保留业务协议，仅替换展示层

- 不修改 API、Prisma schema、生成请求、图片处理协议和 owner 归属逻辑。
- `ConfigSummary` 接收配置项和编辑回调，不写死演示数据。
- 文案和视频入口保留同等视觉权重，但未开放能力只进入占位页。
- 模板页面初始化恢复会话时不清空用户已经开始上传的图片；只有主动切换会话时才重置输入。
